...
MongoDB Compass를 실행하고 shell에서 지금 입력한 내용이 추가되어있는지 확인해 봅니다.   

만약에 collection부분에 books가 아닌 다른 이름을 삽입한다면 어떻게 될까요?   db.뒤에는 컬렉션이라고 했기때문에 실수로 잘못된 이름을 적으면 새로운 컬렉션이 만들어져 있을 것입니다.  
(show collections 를 사용해서 확인해 보시기 바랍니다 )  


insertOne()를 사용해서도 collection를 하나 생성할 수도 있습니다. 
```
